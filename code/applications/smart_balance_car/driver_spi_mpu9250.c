...
volatile uint8_t                g_MPU9250TxBuf[32];
volatile uint8_t                g_MPU9250RxBuf[32];

volatile MPU9250_SensorInfo_T   g_MPU9250GyroOffset = {0, 0, 0};
volatile MPU9250_SensorInfo_T   g_MPU9250AccOffset  = {0, 0, 0};
volatile MPU9250_SensorInfo_T   g_MPU9250AccLast    = {0, 0, 0};
volatile MPU9250_SensorInfo_T   g_MPU9250GyroLast   = {0, 0, 0};
volatile MPU9250_SensorInfo_T   g_MPU9250MagLast    = {0, 0, 0};
...
void mpu8250_value_read(void)
{
    ...
    memset((void *)g_MPU9250TxBuf, 0x00, sizeof(g_MPU9250TxBuf));
    memset((void *)g_MPU9250RxBuf, 0x00, sizeof(g_MPU9250RxBuf));
    g_MPU9250TxBuf[0] = MPU9250_ACCEL_XOUT_H|0x80;
    g_MPU9250TxBuf[1] = 0xFF;
    /* SPI master start transfer */
    g_MPU9250SpiXfer.txData       = (uint8_t *)g_MPU9250TxBuf;
    g_MPU9250SpiXfer.dataSize     = 23;
    g_MPU9250SpiXfer.rxData       = (uint8_t *)g_MPU9250RxBuf;
    g_MPU9250SpiXfer.configFlags |= kSPI_FrameAssert;
	...
	g_MPU9250AccLast.x  = ((int16_t)g_MPU9250RxBuf[ 1]<<8) | g_MPU9250RxBuf[ 2];
	g_MPU9250AccLast.y  = ((int16_t)g_MPU9250RxBuf[ 3]<<8) | g_MPU9250RxBuf[ 4];
	g_MPU9250AccLast.z  = ((int16_t)g_MPU9250RxBuf[ 5]<<8) | g_MPU9250RxBuf[ 6];
    
	g_MPU9250GyroLast.x = ((int16_t)g_MPU9250RxBuf[ 9]<<8) | g_MPU9250RxBuf[10];
	g_MPU9250GyroLast.y = ((int16_t)g_MPU9250RxBuf[11]<<8) | g_MPU9250RxBuf[12];
	g_MPU9250GyroLast.z = ((int16_t)g_MPU9250RxBuf[13]<<8) | g_MPU9250RxBuf[14];
	
	g_MPU9250MagLast.x  = ((int16_t)g_MPU9250RxBuf[17]<<8) | g_MPU9250RxBuf[16];
	g_MPU9250MagLast.y  = ((int16_t)g_MPU9250RxBuf[19]<<8) | g_MPU9250RxBuf[18];
	g_MPU9250MagLast.z  = ((int16_t)g_MPU9250RxBuf[21]<<8) | g_MPU9250RxBuf[20];
	
	MPU9250_CS_SET();
}

// Gyro offset config
void gyro_offect_calc(void)
{
    static int32_t	temp_gx=0,temp_gy=0,temp_gz=0;
    uint16_t cnt_g=0;

    cnt_g = 0;
    g_MPU9250GyroOffset.x=0;
    g_MPU9250GyroOffset.y=0;
    g_MPU9250GyroOffset.z=0;
    temp_gx = 0;
    temp_gy = 0;
    temp_gz = 0;
    ...
    {
        mpu8250_value_read();
        temp_gx+= g_MPU9250GyroLast.x;
        temp_gy+= g_MPU9250GyroLast.y;
        temp_gz+= g_MPU9250GyroLast.z;
        cnt_g++;
        mpu9250_delay(1);
    }
    
    g_MPU9250GyroOffset.x = temp_gx/cnt_g;
    g_MPU9250GyroOffset.y = temp_gy/cnt_g;
    g_MPU9250GyroOffset.z = temp_gz/cnt_g;
}
