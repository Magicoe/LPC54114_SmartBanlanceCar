...
#if BLE_QPP_SERVER || BLE_QPP_CLIENT
void app_eaci_data_req_qpps(uint8_t msg_id, uint8_t param_len, uint8_t const *param)
{
  switch(msg_id)
  {
#if BLE_QPP_SERVER
    case QPPS_DATA_SEND_REQ:
    {
      // Alert Indication
			uint8_t length ;
			int i=0;
			...
			length = param_len + 5;
//			uint8_t* pdu = (uint8_t *) malloc (length * sizeof(uint8_t));

      if (length > 1) {
			  ble_pdu[0] =  EACI_MSG_TYPE_DATA_REQ;
        ble_pdu[1] =  QPPS_DATA_SEND_REQ;
				ble_pdu[2] =  param_len+2;
				ble_pdu[3] = (uint8_t)(ATT_SVC_QPPS_DATA);
				ble_pdu[4] = (uint8_t)(ATT_SVC_QPPS_DATA >> 8);
      ...
      eaci_uart_write(sizeof(uint8_t) * length, (uint8_t *)ble_pdu);
...
#if BLE_QPP_SERVER
static void app_eaci_data_ind_qpps(uint8_t msg_id, uint8_t param_len, uint8_t const *param, void* context)
{
    uint8_t qpps_test_data[20];
    switch (msg_id)
    {
        case QPPS_DAVA_VAL_IND:
            PRINTF("QPPS_DAVA_VAL_IND\r\n");
//          phNfcCreateTLV (context, param_len, param);
//          LPCgotoSleep();                                // LPC go back to sleep
        break;
        case QPPS_CFG_INDNTF_IND:
            PRINTF("QPPS_CFG_INDNTF_IND\r\n");
            bBLENotificationFlag = 1;
...
/***************************************************************************************
 * EACI data indication message handler
 **************************************************************************************/
void app_eaci_data_ind(uint8_t msg_id, uint8_t param_len, uint8_t const *param, void* context)
{
  uint16_t uuid;
    ...
    {
#if BLE_QPP_SERVER
		case ATT_SVC_QPPS_DATA:
			app_eaci_data_ind_qpps(msg_id, param_len-2, param+2, context);
			break;
#endif				
