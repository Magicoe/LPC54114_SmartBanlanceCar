...
void Motor_RefreshPwmController(float AnglePwmOut, float SpeedPwmOut, float TurnPwmOut)
{
    ...
    if (LeftPwmCtrlVal >= 0)
    {
        ...
        motor_speed_set(MOTOR_ID_A, LeftPwmCtrlVal+MOTORA_DEAD_TIME);  //425
    }
    else /* LeftPwmCtrlVal < 0 */
    {
        motor_direction_set(MOTOR_ID_A, eMotor_DirectionBackward);
        motor_speed_set(MOTOR_ID_A, (-LeftPwmCtrlVal)+MOTORA_DEAD_TIME);
    }
#endif
#if 1
    if (RightPwmCtrlVal >= 0)
    {
        ...
        motor_speed_set(MOTOR_ID_B, RightPwmCtrlVal+MOTORB_DEAD_TIME);
    }
    else
    {
        motor_direction_set(MOTOR_ID_B, eMotor_DirectionBackward);
        motor_speed_set(MOTOR_ID_B, (-RightPwmCtrlVal)+MOTORB_DEAD_TIME); //450
    }
#endif
}
